...
if (length(all_models_final_results) > 0) {
  ...
  for (r in 1:max_r_actual) {
    ...
    avg_ranks <- aggregate(rank ~ item_id, data = df_up_to_r, FUN = mean)
    sorted_items <- avg_ranks$item_id[order(avg_ranks$rank, decreasing = FALSE)]
  ...
  output_rds <- file.path(output_dir, paste0("gemini_global_rankings_r", max_r_actual, ".rds"))
