...
target_models <- c("llama3.2", "phi4-mini", "gemma3") # Auto-detected installed models
...
evaluate_block_stateless <- function(target_ids, model = target_models[1], max_retries = 15) {
  ...
  json_template <- paste0(
    "    { \"item_id\": \"", obfuscated_ids, "\", \"reason\": \"<concise_comparative_reason_max_20_words>\", \"rank\": <number_from_1_to_", length(obfuscated_ids), "> }", 
  ...
  prompt_text <- paste0(
    ...
    "CONTEXT:\n", mission_context, "\n\n",
    "DATA (Evaluate these items):\n", toJSON(list(block = block_data_raw), auto_unbox = TRUE, pretty = TRUE), "\n\n",
    "TASK:\n",
    ...
    sprintf("4. CRITICAL: You MUST rank EXACTLY %d items. Ensure ALL these IDs are included: [%s]. DO NOT MISS ANY.", 
            nrow(block_data_raw), paste(obfuscated_ids, collapse=", "))
  )
  ...
  for (attempt in 1:max_retries) {
    ...
    # 決定論陷阱破解法 (Deterministic Trap Breaker)
    # 因為我們強制 temperature = 0，如果模型失敗，重試 100 次也會給一樣的錯誤。
    # 解決方案：若失敗，我們在內部稍微打亂選項的「顯示順序 (Presentation Order)」。
    # 這樣 Prompt 字串會發生改變，強迫模型走不同的神經網路路徑，藉此跳出死胡同。
    if (attempt > 1) {
    ...
    if (status_code(response) == 200) {
      ...
      parsed_data <- tryCatch({
        # Extract only the JSON part using Regex (to remove Llama/Phi preambles)
        clean_content <- regmatches(raw_content, regexpr("(?s)\\{.*\\}", raw_content, perl = TRUE))
        if (length(clean_content) == 0) {
      ...
      if (!is.null(parsed_data)) {
        # Handle key variations (ranking vs rankings)
        rankings_list <- parsed_data$rankings
        if (is.null(rankings_list)) rankings_list <- parsed_data$ranking
        
        if (!is.null(rankings_list)) {
          # Handle ID variations (id vs item_id)
          received_ids_raw <- rankings_list$item_id
          if (is.null(received_ids_raw)) received_ids_raw <- rankings_list$id
          
          if (!is.null(received_ids_raw)) {
            received_ids_upper <- toupper(as.character(received_ids_raw))
            # Extract the letters (A, B, C...) from the received IDs
            ...
            if (length(norm_received) == length(norm_expected) && all(norm_expected %in% norm_received)) {
              ...
              # Map obfuscated letter back to original ID
              mapped_ids <- sapply(norm_received, function(letter) {
                id_map[paste0("Option_", letter)]
              })
              
              # Add position data to the data frame based on the ACTUAL presentation order in THIS attempt
              ...
              rankings_list$item_id <- mapped_ids # Ensure IDs are restored to original
      ...
      message("  ⚠️ API Error: Status ", status_code(response), ". Retrying...")
      Sys.sleep(1) # Give API a moment before retry
    }
  }
  return(NULL)
}
...
for (current_model in target_models) {
  ...
  for (i in seq_along(blocks_list)) {
    ...
    eval_result <- evaluate_block_stateless(target_ids, model = current_model)
...
if (length(all_models_final_results) > 0) {
  ...
  csv_file <- file.path(results_dir, paste0("AI_Detailed_Rankings_MultiModel_", timestamp, ".csv"))
