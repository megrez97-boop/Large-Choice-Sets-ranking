...
# 自動將 RStudio 的工作區切換到本腳本所在的資料夾 (Portable 防呆設計)
try(setwd(dirname(rstudioapi::getActiveDocumentContext()$path)), silent=TRUE)

rbo_path <- "modules/rbo/rbo.R"
gemini_script <- "scripts/01a_API_Gemini_Creative_80.R"
results_dir <- "results"
...
csv_files <- list.files(results_dir, pattern = "AI_Creative_80_Rankings_.*\\.csv", full.names = TRUE)
