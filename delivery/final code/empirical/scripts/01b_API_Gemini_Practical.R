...
excel_path <- file.path(base_dir, "data", "data partition-practical.xlsx")
...
checkpoint_file <- file.path(results_dir, "checkpoint_Practical.csv")
...
if (length(all_models_final_results) > 0) {
  ...
  csv_file <- file.path(results_dir, paste0("AI_Practical_Rankings_", run_timestamp, ".csv"))
