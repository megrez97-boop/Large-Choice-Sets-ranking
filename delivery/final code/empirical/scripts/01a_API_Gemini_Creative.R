...
excel_path <- file.path(base_dir, "data", "data partition-creative.xlsx")
...
checkpoint_file <- file.path(results_dir, "checkpoint_Creative.csv")
...
if (length(all_models_final_results) > 0) {
  ...
  csv_file <- file.path(results_dir, paste0("AI_Creative_Rankings_", run_timestamp, ".csv"))
