...
# Set Working Directory for final thesis plotting
...
sim_worker <- function(args) {
  ...
  # 【重現性關鍵修復】在進行任何隨機操作（包含設計生成與噪音生成）之前，立即設定全局種子以保證重現性
...
run_multi_k_batch <- function(t_val = 120, k_values = c(3, 4, 8, 10), r_limit = 20, 
  ...
  if (file.exists(checkpoint_file)) {
    # 複製成最終檔案，同時保留 checkpoint 供下次重複載入 (Smart Resume)
    file.copy(checkpoint_file, final_backup_name, overwrite = TRUE)
    cat(sprintf("\n💾 原始模擬數據已自動存檔備份至: %s\n", final_backup_name))
  }
  ...
  sidebarLayout(
    ...
    mainPanel(
      tabsetPanel(id = "sim_tabs",
        tabPanel("BT Rho", 
                 plotOutput("btRhoPlot", height = "600px"),
                 br(), downloadButton("download_bt_rho", "💾 Download Plot (PNG)", class = "btn-success"), br(), br()),
        tabPanel("BT Footrule", 
                 plotOutput("btFootPlot", height = "600px"),
                 br(), downloadButton("download_bt_foot", "💾 Download Plot (PNG)", class = "btn-success"), br(), br()),
        tabPanel("PL Rho", 
                 plotOutput("plRhoPlot", height = "600px"),
                 br(), downloadButton("download_pl_rho", "💾 Download Plot (PNG)", class = "btn-success"), br(), br()),
        tabPanel("PL Footrule", 
                 plotOutput("plFootPlot", height = "600px"),
                 br(), downloadButton("download_pl_foot", "💾 Download Plot (PNG)", class = "btn-success"), br(), br()),
        tabPanel("Comp (k=3)", 
                 plotOutput("compK3Plot", height = "600px"),
                 br(), downloadButton("download_comp_k3", "💾 Download Plot (PNG)", class = "btn-success"), br(), br()),
        tabPanel("Comp (k=4)", 
                 plotOutput("compK4Plot", height = "600px"),
                 br(), downloadButton("download_comp_k4", "💾 Download Plot (PNG)", class = "btn-success"), br(), br()),
        tabPanel("Comp (k=8)", 
                 plotOutput("compK8Plot", height = "600px"),
                 br(), downloadButton("download_comp_k8", "💾 Download Plot (PNG)", class = "btn-success"), br(), br()),
        tabPanel("Comp (k=10)", 
                 plotOutput("compK10Plot", height = "600px"),
                 br(), downloadButton("download_comp_k10", "💾 Download Plot (PNG)", class = "btn-success"), br(), br()),
        tabPanel("Computing Time", 
                 ...
                 br(), downloadButton("download_time_comp", "💾 Download Plot (PNG)", class = "btn-success"), br(), br()),
        tabPanel("Total Time", 
                 plotOutput("totalTimePlot", height = "600px"),
                 br(), downloadButton("download_total_time", "💾 Download Plot (PNG)", class = "btn-success"), br(), br()),
        tabPanel("Time by R", 
                 plotOutput("timeByRPlot", height = "600px"),
                 br(), downloadButton("download_time_by_r", "💾 Download Plot (PNG)", class = "btn-success"), br(), br()),
        tabPanel("Summary", 
                 br(),
                 downloadButton("download_summary_csv", "💾 Download Summary Table (CSV)", class = "btn-info"),
                 span(" "),
                 downloadButton("download_raw_csv", "💾 Download Raw Simulation Data (CSV)", class = "btn-info"),
                 br(), br(),
...
server <- function(input, output, session) {
  ...
  # Reactive Plots to share between UI rendering and file downloading
  plot_bt_rho <- reactive({ req(sim_results()); render_band_plot(sim_results(), "rho_bt", "BT", "Spearman Rho") })
  plot_bt_foot <- reactive({ req(sim_results()); render_band_plot(sim_results(), "foot_bt", "BT", "Footrule Error") })
  plot_pl_rho <- reactive({ req(sim_results()); render_band_plot(sim_results(), "rho_pl", "PL", "Spearman Rho") })
  plot_pl_foot <- reactive({ req(sim_results()); render_band_plot(sim_results(), "foot_pl", "PL", "Footrule Error") })
  
  plot_comp_k3 <- reactive({ req(sim_results()); render_comp_band_plot(sim_results(), 3) })
  plot_comp_k4 <- reactive({ req(sim_results()); render_comp_band_plot(sim_results(), 4) })
  plot_comp_k8 <- reactive({ req(sim_results()); render_comp_band_plot(sim_results(), 8) })
  plot_comp_k10 <- reactive({ req(sim_results()); render_comp_band_plot(sim_results(), 10) })

  plot_total_time <- reactive({
    df <- sim_results()
    req(df)
    df_time <- df |>
  ...
  # Render Plots in UI
  output$btRhoPlot <- renderPlot({ plot_bt_rho() })
  output$btFootPlot <- renderPlot({ plot_bt_foot() })
  output$plRhoPlot <- renderPlot({ plot_pl_rho() })
  output$plFootPlot <- renderPlot({ plot_pl_foot() })
  
  output$compK3Plot <- renderPlot({ plot_comp_k3() })
  output$compK4Plot <- renderPlot({ plot_comp_k4() })
  output$compK8Plot <- renderPlot({ plot_comp_k8() })
  output$compK10Plot <- renderPlot({ plot_comp_k10() })
  
  output$timeCompPlot <- renderPlot({ plot_time_comp() })
  output$totalTimePlot <- renderPlot({ plot_total_time() })
  output$timeByRPlot <- renderPlot({ plot_time_by_r() })

  # Download Handlers for Plots
  output$download_bt_rho <- downloadHandler(
    filename = function() { paste0("BT_Rho_t", input$t_val, "_", format(Sys.time(), "%m%d_%H%M"), ".png") },
    content = function(file) { ggsave(file, plot = plot_bt_rho(), width = 10, height = 7, dpi = 300) }
  )
  output$download_bt_foot <- downloadHandler(
    filename = function() { paste0("BT_Footrule_t", input$t_val, "_", format(Sys.time(), "%m%d_%H%M"), ".png") },
    content = function(file) { ggsave(file, plot = plot_bt_foot(), width = 10, height = 7, dpi = 300) }
  )
  output$download_pl_rho <- downloadHandler(
    filename = function() { paste0("PL_Rho_t", input$t_val, "_", format(Sys.time(), "%m%d_%H%M"), ".png") },
    content = function(file) { ggsave(file, plot = plot_pl_rho(), width = 10, height = 7, dpi = 300) }
  )
  output$download_pl_foot <- downloadHandler(
    filename = function() { paste0("PL_Footrule_t", input$t_val, "_", format(Sys.time(), "%m%d_%H%M"), ".png") },
    content = function(file) { ggsave(file, plot = plot_pl_foot(), width = 10, height = 7, dpi = 300) }
  )
  output$download_comp_k3 <- downloadHandler(
    filename = function() { paste0("BT_vs_PL_Comp_k3_", format(Sys.time(), "%m%d_%H%M"), ".png") },
    content = function(file) { ggsave(file, plot = plot_comp_k3(), width = 10, height = 7, dpi = 300) }
  )
  output$download_comp_k4 <- downloadHandler(
    filename = function() { paste0("BT_vs_PL_Comp_k4_", format(Sys.time(), "%m%d_%H%M"), ".png") },
    content = function(file) { ggsave(file, plot = plot_comp_k4(), width = 10, height = 7, dpi = 300) }
  )
  output$download_comp_k8 <- downloadHandler(
    filename = function() { paste0("BT_vs_PL_Comp_k8_", format(Sys.time(), "%m%d_%H%M"), ".png") },
    content = function(file) { ggsave(file, plot = plot_comp_k8(), width = 10, height = 7, dpi = 300) }
  )
  output$download_comp_k10 <- downloadHandler(
    filename = function() { paste0("BT_vs_PL_Comp_k10_", format(Sys.time(), "%m%d_%H%M"), ".png") },
    content = function(file) { ggsave(file, plot = plot_comp_k10(), width = 10, height = 7, dpi = 300) }
  )
  output$download_time_comp <- downloadHandler(
    filename = function() { paste0("Time_Comp_k", input$time_k_select, "_", format(Sys.time(), "%m%d_%H%M"), ".png") },
    content = function(file) { ggsave(file, plot = plot_time_comp(), width = 10, height = 7, dpi = 300) }
  )
  output$download_total_time <- downloadHandler(
    filename = function() { paste0("Total_Time_", format(Sys.time(), "%m%d_%H%M"), ".png") },
    content = function(file) { ggsave(file, plot = plot_total_time(), width = 10, height = 7, dpi = 300) }
  )
  output$download_time_by_r <- downloadHandler(
    filename = function() { paste0("Time_by_R_", format(Sys.time(), "%m%d_%H%M"), ".png") },
    content = function(file) { ggsave(file, plot = plot_time_by_r(), width = 10, height = 7, dpi = 300) }
  )

  # Download Handlers for Datasets (CSV)
  output$download_summary_csv <- downloadHandler(
    filename = function() { paste0("Model_Efficiency_Summary_", format(Sys.time(), "%m%d_%H%M"), ".csv") },
    content = function(file) {
      ...
      write.csv(summary_df, file, row.names = FALSE, fileEncoding = "UTF-8")
    }
  )

  output$download_raw_csv <- downloadHandler(
    filename = function() { paste0("Raw_Simulation_Data_", format(Sys.time(), "%m%d_%H%M"), ".csv") },
    content = function(file) {
      df <- sim_results()
      req(df)
